...
class PurchDisposal extends Component {
  ...
  prosesOpenId = async (val) => {
    ...
    this.setState({
      detailData: detail,
      typeModal: 'edit',
      price: detail.nilai_jual ? detail.nilai_jual : '',
      displayPrice: detail.nilai_jual ? `Rp ${this.formatCurrency(detail.nilai_jual)}` : '',
    });
    this.toggleModal();
  }
  ...
  render() {
    const {time1, time2, showDateFrom, showDateTo, filter, newDis, openDetail, loading, realApp, listMut, listStat, tipeEmail, dataRej, total, detailData, typeModal} = this.state;
    ...
    const loadingAll = loadingDepo || loadingDisposal || loadingSetuju || loadingUser || loadingTempmail || loadingNewnotif || loadingDokumen || loading;
      ...
      <View style={{flex: 1}}>
        ...
        <View style={styles.headerContainer}>
          <View style={styles.bodyTitle}>
            <Text style={styles.title}>Verifikasi Purchasing Disposal</Text>
            {(level === 5 || level === 9) && (
              <TouchableOpacity onPress={() => this.props.navigation.navigate('CartDisposal')}>
            ...
            <ScrollView style={styles.scrollContentModal}>
              ...
              {detailDis.length > 0 && detailDis.map((item, index) => (
                <View key={index} style={[styles.assetCardModal, item.status_app === 0 ? styles.note : listMut.find(element => element === item.id) !== undefined ? styles.note : styles.backgroundWhite]}>
                  ...
                  <TouchableOpacity
                    style={[styles.buttonDoc, styles.btnColorApprove, {marginTop: 10}]}
                    onPress={() => this.prosesOpenId(item)}
                  >
                    <Text style={styles.buttonTextModal}>
                      Filling Nilai Jual
                    </Text>
      ...
      <Modal
        ...
        >
          <EmailModal
            ...
            handleSubmit={tipeEmail === 'reject' ? this.rejectDisposal : this.submitPurchDisposal}
          ...
          <View style={styles.modalContainerDetail}>
            <ScrollView>
              ...
              <View style={styles.formGroupDetail}>
                ...
                <TextInput style={styles.inputDetail} editable={false} value={this.formatCurrency(detailData.nilai_buku)} />
              </View>

              <View style={styles.formGroupDetail}>
                <Text style={styles.labelDetail}>Nilai Jual :</Text>
                <TextInput
              ...
              <View style={styles.buttonWrapperDetail}>
                <TouchableOpacity
                  ...
                  onPress={() => this.cekUpdate(detailData)}
                >
