...
class RevisiDisposal extends Component {
  constructor(props) {
    super(props);
    this.state = {
      loading: false,
  ...
  prosesOpenDetail = async (val) => {
    ...
    const { filter } = this.state;
    await this.props.getKeterangan(token, val.nilai_jual === '' ? null : val.nilai_jual);
  ...
  prosesOpenDokumen = async (val) => {
    const {dataUser, token} = this.props.auth;
    this.setState({detailData: val, typeDoc: 'ajuan'});
    const data = {
        noId: val.id,
  ...
  openDocEksekusi = async (val) => {
    ...
    if (val.npwp !== 'ada' &&  val.npwp !== 'tidak' && val.nilai_jual !== '0') {
      this.setState({detailData: val, confirm: 'falseNpwp'});
      this.openConfirm();
    } else {
      this.setState({typeDoc: 'eksekusi'});
  ...
  saveRevisi = async (val) => {
    ...
    await this.props.updateDisposal(token, detailData.id, data);
    await this.props.appRevisi(token, detailData.id, 'reason');
  ...
  render() {
    ...
    const { dataDis, noDis, dataDoc, detailDis, statusList, dataReason, dataKet} = this.props.disposal;
    ...
    return (
      ...
      <View style={{flex: 1}}>
        ...
        <View style={styles.headerContainer}>
          <View style={styles.bodyTitle}>
            <Text style={styles.title}>Revisi Disposal Asset</Text>
          </View>
          ...
          <View style={styles.popupContainerModal}>
            <ScrollView style={styles.scrollContentModal}>
              ...
              <View style={styles.headerContainerModal}>
                <Text style={styles.headerTitleModal}>Revisi Disposal Asset</Text>
              ...
              {detailDis.length > 0 && detailDis.map((item, index) => (
                <View key={index} style={[styles.assetCardModal, item.status_app === 0 ? styles.note : listMut.find(element => element === item.id) !== undefined ? styles.note : styles.backgroundWhite]}>
                  ...
                  {(item.isreject === 1 || item.isreject === 0) && (
                    <TouchableOpacity
                      style={[styles.buttonDoc, styles.btnColorProses, { marginVertical: 10 }]}
                  ...
                  {(item.status_form !== 26 && item.status_form !== 9 && item.status_form !== 15) && (item.status_form >= 4) && (
                    <TouchableOpacity
      ...
      >
        <EmailModal
          ...
          handleSubmit={this.submitRevDis}
          ...
          <View style={styles.popupContainerInfo}>
            <ScrollView style={styles.scrollContentModal}>
              ...
              parDoc={{
                arrDoc: dataDoc,
                proses: 'upload',
                tipe: this.state.typeDoc === 'eksekusi' ? 'eksekusi disposal' : 'disposal',
        ...
        <ScrollView>
          ...
          <View style={styles.formGroupDetail}>
            ...
            <View style={styles.pickerWrapperDetail}>
              <Picker
                selectedValue={`${detailData.keterangan}`}
                style={styles.pickerDetail}
                onValueChange={(itemValue) => this.setState({detailData: {...detailData, keterangan: itemValue}})}
              >
          ...
          {(detailData.status_form !== 26 && detailData.status_form !== 9 && detailData.status_form !== 15) && (detailData.status_form >= 4) && (
              ...
              <View style={styles.formGroupDetail}>
                ...
                <View style={styles.pickerWrapperDetail}>
                  <Picker
                    ...
                    style={styles.pickerDetail}
                    onValueChange={(itemValue) => this.updateNpwp(itemValue)}
                  >
...
export default connect(mapStateToProps, mapDispatchToProps)(RevisiDisposal);
