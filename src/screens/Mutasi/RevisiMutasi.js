...
class RevisiMutasi extends Component {
  constructor(props) {
    super(props);
    this.state = {
      loading: false,
  ...
  changeFilter = async (val) => {
      ...
      this.setState({ filter: val, newMut: dataMut })
  }
  ...
  prosesOpenDetail = async (val) => {
    ...
    this.setState({loading: false, reason: detailMut[0].alasan});
    this.openDetail();
  }
  ...
  onSearchArea = async () => {
    ...
    const {dataUser, token} = this.props.auth;
    const kode = dataUser.kode_plant;
    const cekFilter = dataArea.filter(x =>
    ...
    this.setState({dataArea: searchArea === '' ? dataDepo.filter(x => x.kode_plant.toLowerCase() !== kode.toLowerCase()) : cekFilter});
  }

  prosesOpenToggle = (val) => {
    this.prepareSelect();
    const area = `${val.kode_plant_rec}-${val.area_rec}`;
    const kode = val.kode_plant_rec;
    this.setState({detailData: val, typeModal: val.type, area: area, kode: kode});
    this.toggleModal();
  }
  ...
  render() {
    const {time1, time2, showDateFrom, showDateTo, filter, newMut, detailData, loading, realApp, listMut, listStat, tipeEmail, dataRej} = this.state;
    ...
    return (
      ...
      <View style={{flex: 1}}>
        ...
        <View style={styles.headerContainer}>
          <View style={styles.bodyTitle}>
            <Text style={styles.title}>Revisi Mutasi Asset</Text>
          </View>
          ...
          <View style={styles.popupContainerModal}>
            <ScrollView style={styles.scrollContentModal}>
              ...
              <View style={styles.sectionModal}>
                ...
                <TextInput
                  style={[styles.inputReason, { marginBottom: 10 }]}
                  placeholder="..........."
                  multiline={true}
      ...
      >
        <EmailModal
          ...
          handleSubmit={this.submitMutasi}
            ...
            <ScrollView style={styles.scrollContentModal}>
              ...
              {this.state.confirm === 'approve' ? (
                  ...
                  <View style={styles.sectionInfo}>
                    <IconMateri name="close" color={'red'} size={50}/>
                    <Text style={styles.sectionTitleInfo}>Gagal Submit</Text>
                    <Text style={[styles.sectioSubtitleInfo]}>Pastikan area tujuan mutasi sama</Text>
                    <TouchableOpacity style={styles.btnInfo} onPress={this.openConfirm}>
                      <Text style={styles.textBtnInfo}>OK</Text>
                    </TouchableOpacity>
                  </View>
          ...
          <ScrollView>
            ...
            <View style={styles.buttonWrapperDetail}>
              <TouchableOpacity
                ...
                onPress={this.state.typeModal === 'add' ? this.prosesAddMutasi : () => this.updateDataMutasi(detailData)}
              >
...
export default connect(mapStateToProps, mapDispatchToProps)(RevisiMutasi);
