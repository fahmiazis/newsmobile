...
class CartPengadaan extends Component {
  constructor(props) {
    super(props);
    this.state = {
      loading: false,
  ...
  handlePrice = (val) => {
    ...
    this.setState({
      price: oriPrice, // angka polosan
      displayPrice: oriPrice ? `Rp ${this.formatCurrency(oriPrice)}` : '',
    });
  };
  ...
  prosesOpenTracking = async (val) => {
    ...
    await this.props.getApproveIo(token, val.no_pengadaan, val.kode_plant.split('').length === 4 ? 'Pengadaan' : 'Pengadaan HO');
  ...
  goCart = () => {
    this.props.navigation.navigate('CartPengadaan');
  }
  ...
  renderData = ({ item }) => (
    <View style={[styles.assetCard, { width: this.state.openList ? cardWidthList : cardWidth }]}>
      ...
      {/* <TouchableOpacity style={styles.btnLabel} onPress={() => this.prosesOpenDelete(item)}>
        <IconMateri name="delete" size={22} color={'white'}/>
      </TouchableOpacity> */}
      <View style={styles.nameContainerCard}>
        <Text style={styles.nameTextCard} numberOfLines={2}>{item.nama}</Text>
      </View>
      <Text style={styles.detailTextCard}>Kategori : {item.kategori}</Text>
      <Text style={styles.detailTextCard}>Jenis : {item.jenis}</Text>
      <Text style={styles.detailTextCard}>Qty : {item.qty}</Text>
      <Text style={styles.detailTextCard}>Price / Unit : {item.price.toString().replace(/\B(?=(\d{3})+(?!\d))/g, '.')}</Text>
  ...
  prosesDeletePengadaan = async (val) => {
      const {dataUser, token} = this.props.auth;
      await this.props.deleteCart(token, val.id);
      await this.props.getCart(token);
      this.openDelete();
      ...
      <View style={{ flex: 1 }}>
        ...
        <View style={styles.headerContainer}>
          <View style={styles.bodyTitle}>
            <Text style={styles.title}>Draft Pengadaan Asset</Text>
          </View>
          ...
          <View style={styles.dateRow}>
            <TouchableOpacity
              ...
              onPress={() => this.prosesOpenDetail({val: {}, type: 'add'})}
              >
          ...
          <View style={styles.popupContainerModal}>
            <ScrollView style={styles.scrollContentModal}>
              ...
              <View style={styles.infoCardModal}>
                <Text style={styles.infoTextModal}>Tanggal Form: {detailIo.length > 0 ? moment(detailIo[0].tanggalIo).format('DD MMMM YYYY') : '-'}</Text>
                <Text style={styles.infoTextModal}>Tanggal Pengadaan Fisik: {detailIo.length > 0 ? (detailIo[0].tgl_pengadaanfisik === null ? '-' : moment(detailIo[0].tgl_pengadaanfisik).format('DD MMMM YYYY')) : '-'}</Text>
                <Text>{''}</Text>
                <Text style={styles.infoTextModal}>Cabang/Depo Asal: {detailIo.length > 0 ? detailIo[0].area : ''}</Text>
                <Text style={styles.infoTextModal}>Cost Center Asal: {detailIo.length > 0 ? detailIo[0].cost_center : ''}</Text>
                <Text>{''}</Text>
                <Text style={styles.infoTextModal}>Cabang/Depo Tujuan: {detailIo.length > 0 ? detailIo[0].area_rec : ''}</Text>
                <Text style={styles.infoTextModal}>Cost Center Tujuan: {detailIo.length > 0 ? detailIo[0].cost_center_rec : ''}</Text>
              </View>
              ...
              <View style={styles.sectionModal}>
                ...
                <View style={styles.assetCardModal}>
                    <Text style={styles.infoTextModal}>{detailIo.length > 0 ? detailIo[0].alasan : ''}</Text>
                </View>
              </View>
      ...
      >
        <View style={styles.modalContainerDetail}>
          <ScrollView>
            ...
            <View style={styles.formGroupDetail}>
              <Text style={styles.labelDetail}>Tipe :</Text>
              <View style={styles.pickerWrapperDetail}>
                ...
                >
                  ...
                  <Picker.Item value={'barang'} label={'Barang'} />
                </Picker>
              </View>
              {tipe === '' && (
                <Text style={styles.errorTextDetail}>Must be filled</Text>
            ...
            <View style={styles.formGroupDetail}>
              <Text style={styles.labelDetail}>Jenis IT / NON IT :</Text>
              <View style={styles.pickerWrapperDetail}>
                <Picker
                  selectedValue={jenis}
                  style={styles.pickerDetail}
                  onValueChange={(itemValue) => this.setState({jenis: itemValue})}
                >
                  <Picker.Item label="Select..." value="" />
            ...
            <View style={styles.formGroupDetail}>
              <Text style={styles.labelDetail}>Price :</Text>
              <TextInput
            ...
            <View style={styles.formGroupDetail}>
              ...
              <View style={styles.pickerWrapperDetail}>
                <Picker
                  ...
                  style={styles.pickerDetail}
                  onValueChange={(itemValue) => this.setState({kategori: itemValue})}
                >
                  <Picker.Item label="Select..." value="" />
          ...
          <View style={styles.popupContainerInfo}>
            <ScrollView style={styles.scrollContentModal}>
              ...
              {this.state.confirm === 'add' ? (
                ...
                ) : (this.state.confirm === 'submitAdd') ? (
                  <View style={styles.sectionInfo}>
                    <IconMateri name="close" color={'red'} size={50}/>
                    <Text style={styles.sectionTitleInfo}>Gagal Submit</Text>
                    <Text style={[styles.sectioSubtitleInfo]}>Tambahkan data terlebih dahulu</Text>
                    <TouchableOpacity style={styles.btnInfo} onPress={this.openConfirm}>
                      <Text style={styles.textBtnInfo}>OK</Text>
                    </TouchableOpacity>
                  </View>
                  ...
                  <View style={styles.sectionInfo}>
                    <IconMateri name="check-circle" color={'green'} size={50}/>
                    <Text style={styles.sectionTitleInfo}>Berhasil Submit Pengadaan</Text>
                    <TouchableOpacity style={styles.btnInfo} onPress={this.openConfirm}>
                      <Text style={styles.textBtnInfo}>OK</Text>
                    </TouchableOpacity>
                  </View>
            ...
            <ScrollView style={styles.scrollContentModal}>
              <View style={styles.sectionInfo}>
                ...
                <View style={styles.sectionDelete}>
                  <TouchableOpacity style={[styles.buttonDelete, styles.btnColorClose]} onPress={() => this.prosesDeletePengadaan(detailData)}>
          ...
          <View style={styles.popupContainerInfo}>
            <ScrollView style={styles.scrollContentModal}>
              <View style={styles.sectionInfo}>
                ...
                <View style={styles.sectionDelete}>
                  <TouchableOpacity style={[styles.buttonDelete, styles.btnColorClose]} onPress={dataCart.length > 0 && dataCart[0].kategori === 'return' ? () => this.openReason() : () => this.prepSubmit()}>
                    <Text style={styles.buttonTextCard}>Ya</Text>
                  </TouchableOpacity>
          ...
          <View style={styles.popupContainerInfo}>
            <ScrollView style={styles.scrollContentModal}>
              <View style={styles.sectionReason}>
                <Text style={styles.titleReason}>Alasan Return : </Text>
                <TextInput
