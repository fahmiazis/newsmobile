...
class RevisiPengadaan extends Component {
  constructor(props) {
    super(props);
    this.state = {
      loading: false,
  ...
  prosesOpenToggle = (val) => {
    if (val.nama !== undefined) {
      this.setState({
        ...
        tipe: val.tipe,
        jenis: val.jenis,
        displayPrice: val.price ? `Rp ${this.formatCurrency(val.price)}` : '',
      });
    }
    this.setState({detailData: val, typeModal: val.type});
    this.toggleModal();
  }
  ...
  prosesUpdatePengadaan = async () => {
    ...
    for (let i = 0; i < detailIo.length; i++) {
        if ((detailIo[i].kategori !== data.kategori || detailIo[i].tipe !== data.tipe) && detailIo.length > 1) {
            cek.push(1);
        }
    }
    if (cek.length > 0) {
  ...
  selectSubmit = () => {
    ...
    } else if (tipeEmail === 'budget') {
      this.submitBudget();
    } else if (tipeEmail === 'reject') {
      this.rejectPengadaan(dataRej);
    } else if (tipeEmail === 'approve') {
      this.approvePengadaan();
    }
  }
  ...
  submitAsset = async (val) => {
    ...
    const cekDok = [];
    const { detailIo } = this.props.pengadaan;
    console.log('masuk submit asset');
    for (let i = 0; i < detailIo.length; i++) {
      if (detailIo[i].isAsset !== 'true' && detailIo[i].isAsset !== 'false') {
        cek.push(detailIo[i]);
      } else if (detailIo[i].isAsset === 'false') {
  ...
  render() {
    ...
    const { nama, qty, price, kategori, tipe, jenis } = this.state;
    ...
    const { dataPeng, noDis, dataDoc, detailIo, statusList, dataDocCart} = this.props.pengadaan;
    ...
    return (
      ...
      <View style={{flex: 1}}>
        ...
        <View style={styles.headerContainer}>
          <View style={styles.bodyTitle}>
            <Text style={styles.title}>Revisi Pengadaan Asset</Text>
          </View>
            ...
            <ScrollView style={styles.scrollContentModal}>
              ...
              {detailIo.length > 0 && detailIo.map((item, index) => (
                <View key={index} style={[styles.assetCardModal, item.status_app === 0 ? styles.note : listMut.find(element => element === item.id) !== undefined ? styles.note : styles.backgroundWhite]}>
                  ...
                  <View style={styles.assetRowModal}>
                    <Text style={styles.assetLabelModal}>Status Data:</Text>
                    <Text style={styles.assetValueModal}>{(item.isreject === 1 || item.isreject === 0) ? (item.isreject === 1 ? 'Perlu Diperbaiki' : 'Telah diperbaiki') : '-'}</Text>
                  </View>
                  {(item.isreject === 1 || item.isreject === 0) &&
                    <TouchableOpacity
                      style={[styles.buttonDoc, styles.btnColorApprove, { marginVertical: 10 }]}
                      onPress={() => this.prosesOpenToggle(item)}
                    >
                      <Text style={styles.buttonTextModal}>
                        Update
                      </Text>
              ...
              <View style={styles.sectionModal}>
                ...
                />
                {/* </View> */}
                {(level === 5 || level === 9) && filter === 'available' && (detailIo.length > 0 && detailIo[0].status_form !== '1') ? (
                  <TouchableOpacity
                    style={[styles.buttonDoc, styles.btnColorApprove]}
      ...
      >
        <EmailModal
          ...
          handleSubmit={this.submitRevisi}
          ...
          <View style={styles.popupContainerInfo}>
            <ScrollView style={styles.scrollContentModal}>
              ...
              {this.state.confirm === 'approve' ? (
                  ...
                  <View style={styles.sectionInfo}>
                    <IconMateri name="close" color={'red'} size={50}/>
                    <Text style={styles.sectionTitleInfo}>Gagal Megupdate Item</Text>
      ...
      >
        <View style={styles.modalContainerDetail}>
          <ScrollView>
            {/* Header */}
            <Text style={styles.headerTextDetail}>Revisi Item</Text>
            ...
            <View style={styles.buttonWrapperDetail}>
              <TouchableOpacity
                ...
                onPress={() => this.prosesUpdatePengadaan()}
              >
...
export default connect(mapStateToProps, mapDispatchToProps)(RevisiPengadaan);
